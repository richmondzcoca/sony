...
export class InformationReturnedSamplesComponent implements OnInit {
    ...
    returnedSamplesData: wjcCore.CollectionView;
    selectedItems: any[] = [];
    ...
    initializeGrid(flex: wjcGrid.FlexGrid): void {
        console.log('initialized......')
        const tt = new wjcCore.Tooltip();
        flex.formatItem.addHandler((s, e) => {
          tt.setTooltip(e.cell, e.cell.innerText, wjcCore.PopupPosition.Right);
        });
        flex.allowMerging = wjcGrid.AllowMerging.All;
        flex.columnHeaders.rows[1].allowMerging = true;
      }
    ...
    deleteIrsRecord(x : number): void {
      this.selectedItems = this.returnedSamplesData.items.filter(item => item.scope);
            ...
            .then((res: SweetAlertResult) => {
                if (res.isConfirmed) {
                  ...
                  this.selectedItems.forEach((item) => {
                    if(item.scope){
                        this.returnedSamplesData.remove(item)
                    }
